	...
	useEffect(() => {
		...
		const listener = () => {
			...
			for (const pc of storage.peerConnections()) {
				if (pc.updates.avgRttInS > 0) {
					rttInMs += pc.updates.avgRttInS * 1000;
				}
			}
			const stats: Stats[] = [ {
				name: 'Downstream Audio',
				value: `${Math.ceil(storage.updates.receivingAudioBitrate / 1000)} kbps`,
			}, {
				name: 'Downstream Video',
				value: `${Math.ceil(storage.updates.receivingVideoBitrate / 1000)} kbps`,
			}, {
				name: 'Upstream Audio',
				value: `${Math.ceil(storage.updates.sendingAudioBitrate / 1000)} kbps`,
			}, {
				name: 'Upstream Video',
				value: `${Math.ceil(storage.updates.sendingVideoBitrate / 1000)} kbps`,
			}, {
