...
// Whether this browser can do E2EE (WebRTC Encoded Transform). Used to gate the feature and to refuse
// admission to a room that mandates E2EE. Chrome, Edge and Firefox all encrypt and interoperate
// (verified 2026-08-31), so this is a plain capability check rather than an engine allowlist. A
// browser that accepts the transform but never feeds it is caught at runtime instead: the sender
// holds real media until the worker confirms it is processing frames, and the watchdog removes the
// participant if that never happens.
